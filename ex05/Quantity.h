...
class Quantity {
	public:
		...
		typedef Quantity<T, N, MASS, LENGTH, TIME-1> time_derivative_type; //< Type of the temporal derivative.
...
class Quantity<T, 1, MASS, LENGTH, TIME> {
	public:
		typedef T value_type; //< Basic data type.
		typedef Quantity<T, 1, MASS, LENGTH, TIME-1> time_derivative_type; //< Type of the temporal derivative.
		 ...
		 */
		bool operator<(const Quantity<T, 1, MASS, LENGTH, TIME> &other) const {
...
class Quantity<T, 1, 0, 0, 0> {
	public:
		typedef T value_type; //< Basic data type.
		typedef Quantity<T, 1, 0, 0, -1> time_derivative_type; //< Type of the temporal derivative.
		 ...
		 * Less-than operator for scalar quantities.
		 */
		bool operator<(const Quantity<T, 1, 0, 0, 0> &other) const {
			return _value < other._value;
		}
...
template <typename T, int MASS, int LENGTH, int TIME>
Quantity<T, 1, MASS, LENGTH, TIME>
abs(const Quantity<T, 1, MASS, LENGTH, TIME> &q1) {
	return Quantity<T, 1, MASS, LENGTH, TIME>(std::abs(q1.value()));
}
...
typedef Quantity<double, 2, 0, 1, 0> Length2D; //< A two-dimensional length. Example (using C++0x): Length2D l = {0.0 * m, 1.0 * m};
...
// Angular quantities:
typedef Quantity<double, 1, 0, 0, 0> Angle;
typedef Quantity<double, 1, 0, 0, -1> AngularVelocity;
typedef Quantity<double, 1, 0, 0, -2> AngularAcceleration;
